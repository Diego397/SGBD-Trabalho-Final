...
def process_operation(line, operations_table, wait_table, wait_graph, aborted_transactions_list, lsn):
	if (line[0] != 'i'):
		...
		if operation_type == "irl":
			...
			for operation in operations_table:
				if (
					operation[2] != transaction_id
					and operation[3] == obj
					and (operation[4] == "cl")
				):
					status = 3
					wait_table.append([lsn, transaction_id, operation[1], operation[2]])
					wait_graph.add_edge(transaction_id, operation[2])
		...
		if operation_type == "iwl":
			...
			for operation in operations_table:
				if (
					operation[2] != transaction_id
					and operation[4] == obj
					and (operation[5] == "wl" or operation[4] == "cl")
				):
					status = 3
					wait_table.append([lsn, transaction_id, operation[1], operation[2]])
					wait_graph.add_edge(transaction_id, operation[2])
		...
		if operation_type == "icl":
			for write_operation in operations_table:
				status = 1
				if (
					write_operation[2] == transaction_id
					and write_operation[4] == "wl"
				):
					obj = write_operation[3]
					for operation in operations_table:
						if (
							operation[2] != transaction_id
							and operation[3] == obj
							and (operation[4] == "wl" or operation[4] == "cl" or operation[4] == "rl")
						):
							status = 3
							wait_table.append([lsn, transaction_id, operation[1], operation[2]])
							wait_graph.add_edge(transaction_id, operation[2])

					operations_table.append([lsn, transaction_id, write_operation[4], operation_type, status])
					lsn += 1
	return lsn
